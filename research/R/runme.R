...
# plotHistogramByFinishers(scoresDF %>% filter(GibushMonth=='××•×’'),param_name = 'AvgScore','××•×’')
# plotHistogramByFinishers(scoresDF %>% filter(GibushMonth=='× ×•×‘'),param_name = 'AvgScore','× ×•×‘')
# plotHistogramByFinishers(scoresDF %>% filter(GibushMonth=='×ž×¨×¥'),param_name = 'AvgScore','×ž×¨×¥')
...
plotHistogramByFinishersCustomFacet(scoresDF,param_name = 'AvgScore',postfix = '×œ×™×‘×” ×ž×•×œ ×œ× ×œ×™×‘×”',facet_formula = formula("Finished~ Liba"))

#per mitam
perMitamTest(finisherScores,nonFinisherScores)

#Mitam 0 plot
plotHistogramByFinishers(scoresDF %>% filter(as.numeric(as.character(Mitam))<2),param_name='AvgScore','ìà ìéáä')
#Mitam 1,2 plot
plotHistogramByFinishers(scoresDF %>% filter(as.numeric(as.character(Mitam))>1),param_name='AvgScore', 'ìéáä')
...
plotConnectionBetweenTraitsAndMetric(avgScoreCorreation, '×ž×ª×× ×‘×™×Ÿ ×ª×›×•× ×•×ª ×œ×¦×™×•×Ÿ ×ž×¢×¨×™×›×™× ×ž×ž×•×¦×¢')
...
spearman_liba <- plotConnectionBetweenTraitsAndMetric(unitSuitabilityCorrelation_liba, 'ìéáä: äîúàí ùì ëì úëåðä åùì öéåï äúàîä ìéçéãä')
spearman_nonliba <- plotConnectionBetweenTraitsAndMetric(unitSuitabilityCorrelation_nonliba, 'ìà ìéáä: äîúàí ùì ëì úëåðä åùì öéåï äúàîä ìéçéãä')
