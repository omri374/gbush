...
traitsPlot <- function(raw,title = 'ALL'){
  ...
  ggplot(melted, aes(x=as.factor(value),fill = FinishedMaslul))  +
    geom_bar(aes(fill = FinishedMaslul)) + facet_grid(variable~.) +xlab("") +ggtitle(title) + ggtech::theme_tech(theme="airbnb")
}
...
## Per mitam test
perMitamTest <- function(finishers,nonFinishers){
  nonLiba <- 0
  liba <- c(1,2)
...
## Per month test
perMonthTest <- function(finishers,nonFinishers){
